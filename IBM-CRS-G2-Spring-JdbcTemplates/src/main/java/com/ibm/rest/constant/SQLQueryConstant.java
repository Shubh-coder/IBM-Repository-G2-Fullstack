...
public class SQLQueryConstant {
	...
	public static final String LOGIN_USER="select * from user where Email= ? and Password=? ";
	...
	public static final String INSERT_PAYMENT="insert into payment_fees(Payment_Method,Total_Amount,Student_Id)values(?,?,?)";
	...
	public static final String STUDENT_TOTAL_AMOUNT = "select sum(Course_Price) from course where Id in (select course_id from course_reg where student_id = ?)";
	public static final String COURSE_F="select sum(Course_Price) from course where Id in (select course_id from course_reg where student_id = ?) ";
	public static final String INSERT_STUDENT_DETAIL="insert into student(Name,Qualification,Branch,Contact,Address)values(?,?,?,?,?)";
	public static final String VIEW_GRADE="select id,grade from student where Id = ?";
